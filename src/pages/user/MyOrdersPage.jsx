...
const MyOrdersComponent = () => {
  ...
  const { orders, loading, cancelLoading, cancelError, myOrdersTotalPages, myOrdersLimit } = useSelector(
    (state) => state.order
  );
  ...
  useEffect(() => {
    if (user?._id) {
      dispatch(fetchMyOrders({ userId: user?._id, accessToken, page: currentPage, limit: myOrdersLimit }));
    }
  ...
  const handlePageChange = (page) => {
    setCurrentPage(page);
    if (user?._id) {
      dispatch(fetchMyOrders({ userId: user?._id, accessToken, page, limit: myOrdersLimit }));
    }
  };
